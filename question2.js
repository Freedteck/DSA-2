...
class Staff {
  ...
  static getStaffs(sortedBy = 1) {
    if (sortedBy == 1) {
      return Staff.staffs.sort((staffA, staffB) =>
        staffA.firstName.localeCompare(staffB.firstName)
      );
    } else if (sortedBy == 2) {
      return Staff.staffs.sort((staffA, staffB) =>
        staffA.lastName.localeCompare(staffB.lastName)
      );
    } else if (sortedBy == 3) {
...
readInterface.question(
  ...
  (answer) => {
    ...
    for (let i = 0; i < sortedStaff.length; i++) {
      console.log(
        `${i + 1}. ${sortedStaff[i].firstName} ${sortedStaff[i].lastName} ${
          sortedStaff[i].age
        }`
      );
    }
