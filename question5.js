    ...
    if (word == "1") {
      readInterface.question("Enter account number: ", (accountNumber) => {
        if (!Account.accountExist(accountNumber)) {
          ...
          console.log(
            `The account balance for ${account.firstName} ${account.lastName} is: ${account.balance}\n`
          );
        }
      ...
      readInterface.question("Enter account number: ", (accountNumber) => {
        if (!Account.accountExist(accountNumber)) {
          ...
          let account = Account.getAccountDetails(accountNumber);
          console.log(
            `The account details for account number ${accountNumber} is : [‘First Name: ${account.firstName}’, ‘Last Name: ${account.lastName}’, ‘Phone Number: ${account.phoneNumber}’]\n`
          );
        }
        readInterface.prompt();
      });
      ...
      readInterface.question("Enter account number: ", (accountNumber) => {
        if (!Account.accountExist(accountNumber)) {
          ...
          let transactions =
            Account.getAccountDetails(accountNumber).transactions;
          ...
          readInterface.question(
            ...
            (previousNumber) => {
              if (account.phoneNumber != previousNumber) {
                console.log(
                  `Invalid phone number. This is not the correct phone number for ${account.firstName} ${account.lastName}. Please enter the correct phone number.\n`
                );
              } else {
                readInterface.question(
